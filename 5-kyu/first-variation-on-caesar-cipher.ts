...
/* 
    ...
    1 is shorter. If the length is 16 the parts will be of lengths 4, 4, 4, 4, 0. 
    Parts 1, 2, 3, 4 are evenly split and the fifth runner will stay at home since his 
    part is the empty string. If the length is 11, equal parts would be of length 2.2, 
...
export const movingShift = (s: string, shift: number) => {
  const shifted = s
    .split("")
    .map((c, i) => (/[a-z]/gi.test(c) ? trans(c, shift + i) : c))
    .join("");
  const size = Math.ceil(shifted.length / 5);
  return [...Array(5)].map((_, i) => shifted.slice(i * size, ++i * size));
};
...
); // should => ["J vltasl rlhr ", "zdfog odxr ypw", " atasl rlhr p ", "gwkzzyq zntyhv", " lvz wp!!!"]
