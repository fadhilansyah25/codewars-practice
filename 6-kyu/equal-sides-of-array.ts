...
export function findEvenIndex(arr: number[]): number {
  ...
  while (i < arr.length) {
    if (i == 0) {
      if (arr.slice(1).reduce(reducer, 0) == i) return i;
    } else {
      if (
        arr.slice(0, i).reduce(reducer, 0) ==
...
// Manual Test
console.log(findEvenIndex([])); // should =>  -1,
console.log(findEvenIndex([1, 2, 3, 4, 3, 2, 1])); // should =>  3
console.log(findEvenIndex([1, 100, 50, -51, 1, 1])); // should => 1
console.log(findEvenIndex([1, 2, 3, 4, 5, 6])); // should => -1
console.log(findEvenIndex([20, 10, 30, 10, 10, 15, 35])); // should => 3
