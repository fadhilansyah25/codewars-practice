...
/* 
    ...
    #What to do
    Your task is to write a function that will encrypt a phrase using Caesar Cipher and 
    a given shift key and return it as a string. However, the key should be increased by 1 
    with every word in a phrase. For example, if the function is called with a phrase 
...
// Solution
function caesarEncode(phrase: string, shift: number) {
  return phrase
    .split(" ")
    .map((w, i) => {
      return w
        .split("")
        .map((c) =>
          String.fromCharCode(((c.charCodeAt(0) - 97 + (shift + i)) % 26) + 97)
        )
        .join("");
    })
    .join(" ");
...
console.log(caesarEncode("fere libenter homines id quod volunt credunt", 7));
